                ...
                <ResponsiveContainer width="100%" height={h}>
                  <PieChart margin={{ top: 20, bottom: 20, left: 40, right: 40 }}>
                    <Pie 
                      ...
                      cy="50%" 
                      innerRadius={0} 
                      outerRadius={isZoomed ? 140 : 70} 
                      paddingAngle={4} 
                      dataKey="value" 
                      label={({ name, value, percent }) => `${name} ${formatWan(value)} (${(percent * 100).toFixed(1)}%)`}
                      labelLine={true}
                    >
                ...
                <ResponsiveContainer width="100%" height={h}>
                  <PieChart margin={{ top: 20, bottom: 20, left: 40, right: 40 }}>
                    <Pie 
                      ...
                      outerRadius={isZoomed ? 140 : 70} 
                      dataKey="value" 
                      label={({ name, value, percent }) => `${name} ${value}项 (${(percent * 100).toFixed(1)}%)`}
                      labelLine={true}
                    >
                      { data.map((_:any, i:number) => <Cell key={i} fill={COLORS[i % COLORS.length]} />) }
                    </Pie>
